...
**Stage 2 — `scripts/lookup_npis.py`**: Queries the NPPES (National Provider Identifier) registry to find NPI numbers, credentials, specialties, and practice locations for each author. Filters to relevant specialties (PM&R, pain medicine, neurology, orthopedic surgery, neurosurgery, anesthesiology/pain).
...
uv run main.py --state IL --top 200        # Run all 7 stages, check top 200 against Anthem
...
uv run scripts/lookup_npis.py              # Stage 2: ~5 min, hits NPPES API
uv run scripts/lookup_npis.py --all        # Include international authors (slower)
