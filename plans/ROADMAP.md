...
- **Install path.** The experiment uses Ollama, which `uv` can't manage (Go binary + daemon, not a Python package) — users have to run Ollama's own cross-platform installer before `uv sync` is any use. Two ways out: (a) bundle an Ollama install check into `main.py` and give clear download instructions; or (b) switch to an in-process uv-installable runtime — `llama-cpp-python`, `candle`, or similar — so `uv sync` is the only install step. (b) is cleaner for this project's conventions but requires redoing the model-loading/API layer and picking a GGUF source.
